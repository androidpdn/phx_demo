defmodule Mix.Tasks.Tailwind.Gen.Context do
  ...
  Generates a context with functions around an Ecto schema.
      ...
      mix tailwind.gen.context Accounts User users name:string age:integer
  ...
  a `--table` option may be provided. For example:
      ...
      mix tailwind.gen.context Accounts User users --table cms_users
  ...
  def run(args) do
    if Mix.Project.umbrella?() do
      Mix.raise("mix tailwind.gen.context can only be run inside an application directory")
    end
  ...
  defp inject_schema_access(%Context{file: file} = context, paths, binding) do
    unless Context.pre_existing?(context) do
      Mix.Generator.create_file(
        file,
        Mix.Phoenix.eval_from(paths, "priv/templates/tailwind.gen.context/context.ex", binding)
      )
    end

    paths
    |> Mix.Phoenix.eval_from(
      "priv/templates/tailwind.gen.context/#{schema_access_template(context)}",
      binding
    )
  ...
  defp inject_tests(%Context{test_file: test_file} = context, paths, binding) do
    unless Context.pre_existing_tests?(context) do
      Mix.Generator.create_file(
        ...
        Mix.Phoenix.eval_from(
          paths,
          "priv/templates/tailwind.gen.context/context_test.exs",
          binding
        )
      )
    end
    ...
    paths
    |> Mix.Phoenix.eval_from("priv/templates/tailwind.gen.context/test_cases.exs", binding)
  ...
  def raise_with_help(msg) do
    ...
    mix tailwind.gen.html, tailwind.gen.json, tailwind.gen.live, and tailwind.gen.context
    ...
    For example:
        ...
        mix tailwind.gen.html Accounts User users name:string
        mix tailwind.gen.json Accounts User users name:string
        mix tailwind.gen.live Accounts User users name:string
        mix tailwind.gen.context Accounts User users name:string
